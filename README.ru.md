...
| [Виджеты](#виджеты)                                      | Четыре custom elements и их `wiring`                  |
...
```sh
npm i @evgkch/fsmjs-inspector
```

Пакет поставляется только в формате ESM. Peer-зависимости — `@evgkch/fsmjs >= 0.2.0` и `@evgkch/channeljs >= 1.1.0`: в сборку пакета они не входят, и копия `fsmjs` у приложения должна быть одна (см. [Ограничения](#ограничения)).
...
**Готовая страница.** Клонируйте репозиторий и запустите:
...
**Работающий автомат.** Инспектор запускается отдельно, а в отлаживаемое приложение добавляется одна строка:
...
**На своей странице.** Фигура встраивается в отлаживаемую страницу — целиком через [`overlay`](#evgkchfsmjs-inspectorui) или по одному виджету (см. [Виджеты](#виджеты)):
...
Приложение с машинами и страница наблюдения — два клиента; реле соединяет их.
...
В `schemas/` лежат десять готовых файлов. Семь — `the-inspectors-*` — схемы машин самого инспектора, выписанные из его исходников командой `npm run dump`. Три написаны вручную: `selection-rectangle`, `upload-with-retry` и `a-schema-with-problems` — на последней видно, как показываются мёртвые правила и недостижимые состояния.
...
- **По проводу идут только имена.** Условия дампа не выполняются: страница наблюдения отправляет событие, а какое правило сработает, решает машина в своём процессе. Контекст и `payload` не покидают приложение.
