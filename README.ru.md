...
Пакет поставляется только в формате ESM. Peer-зависимости — `@evgkch/fsmjs >= 0.2.1` и `@evgkch/channeljs >= 1.1.0`: в сборку пакета они не входят, и копия `fsmjs` у приложения должна быть одна (см. [Ограничения](#ограничения)).
