...
Имя переключателя — тег без `fsmjs-`; нескольким виджетам одного тега имя задаётся вторым аргументом. `desk.ensemble` — связка (`fire`, `rewind`, `forget`, `draw`); `desk.panels` — машина панелей, для страницы со своей раскладкой.
